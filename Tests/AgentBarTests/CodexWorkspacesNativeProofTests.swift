...
/// Opt-in interactive proof in a signed XCTest host, never normal AgentBar startup.
@MainActor
final class CodexWorkspacesNativeProofTests: XCTestCase {
    func test_nativeMenuAndWindowLifecycle() throws {
        let environment = ProcessInfo.processInfo.environment
        guard let directoryPath = environment["AGENTBAR_WORKSPACES_PROOF_DIR"] else {
            throw XCTSkip("Set AGENTBAR_WORKSPACES_PROOF_DIR for isolated native UI proof.")
        }
        guard SettingsStore.isRunningTests,
              environment["AGENTBAR_SUPPRESS_TEST_KEYCHAIN_ACCESS"] == "1",
              environment[CodexCredentialFileAccess.isolationEnvironmentKey] == "1",
              environment["AGENTBAR_ALLOW_TEST_KEYCHAIN_ACCESS"] != "1",
              environment["AGENTBAR_TEST_CODEX_FILE_FIXTURES"] == nil
        else {
        ...
        let host = NSWindow(
            ...
            backing: .buffered,
            defer: false)
        host.title = "AgentBar Workspaces Proof"

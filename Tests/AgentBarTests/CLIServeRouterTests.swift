...
struct CLIServeRouterTests {
    ...
    func `usage operation fingerprint separates dashboard account mode`() {
        let allAccounts = AgentBarCLI.serveUsageOperationFingerprint(
            configFingerprint: "config",
            includeAllCodexAccounts: true)
        let selectedAccount = AgentBarCLI.serveUsageOperationFingerprint(
            configFingerprint: "config",
            includeAllCodexAccounts: false)

        #expect(allAccounts != selectedAccount)
        #expect(allAccounts == AgentBarCLI.serveUsageOperationFingerprint(
            configFingerprint: "config",
            includeAllCodexAccounts: true))
    }
    ...
    func `health response reports ok status and build version`() throws {
        let response = AgentBarCLI.serveHealthResponse(version: "1.2.3")
    ...
    func `health response omits version detail when unavailable`() throws {
        let response = AgentBarCLI.serveHealthResponse(version: nil)
        #expect(response.status == .ok)
        let object = try JSONSerialization.jsonObject(with: response.body) as? [String: Any]
        #expect(object?["status"] as? String == "ok")
    ...
    func `serve numeric options reject malformed values`() {
        #expect(AgentBarCLI.decodeServePort(from: ParsedValues(
            positional: [],
            options: ["port": ["abc"]],
            flags: [])) == nil)
        #expect(AgentBarCLI.decodeServePort(from: ParsedValues(
        ...
        #expect(AgentBarCLI.decodeServeRefreshInterval(from: ParsedValues(
            positional: [],
        ...
        #expect(AgentBarCLI.decodeServeRequestTimeout(from: ParsedValues(
            positional: [],
    ...
    func `serve help documents request timeout option`() {
        let serve = AgentBarCLI.serveHelp(version: "0.0.0")
        let root = AgentBarCLI.rootHelp(version: "0.0.0")

        #expect(serve.contains("--request-timeout <seconds>"))
        #expect(serve.contains("agentbar serve --port 8080 --refresh-interval 60 --request-timeout 30"))
    ...
    func `serve config snapshot reflects provider changes`() throws {
        ...
        var firstConfig = AgentBarConfig.makeDefault()
        firstConfig.setProviderConfig(ProviderConfig(id: .opencodego, enabled: false))
        try store.save(firstConfig)

        let firstSnapshot = try AgentBarCLI.loadServeConfigSnapshot(configStore: store)
        ...
        let secondSnapshot = try AgentBarCLI.loadServeConfigSnapshot(configStore: store)
        ...
        let operationKey = try AgentBarCLI.serveOperationKey(kind: "usage", provider: nil)
        #expect(try operationKey == (AgentBarCLI.serveOperationKey(kind: "usage", provider: nil)))
        #expect(
            AgentBarCLI.serveCacheKey(operationKey: operationKey, configToken: firstSnapshot.cacheToken) !=
                AgentBarCLI.serveCacheKey(operationKey: operationKey, configToken: secondSnapshot.cacheToken))
    }
    ...
    func `serve cache skips provider error payloads`() {
        ...
        #expect(AgentBarCLI.shouldCacheServeResponse(success))
        #expect(!AgentBarCLI.shouldCacheServeResponse(providerError))
        #expect(!AgentBarCLI.shouldCacheServeResponse(routeError))
    }

    @Test
    func `serve provider timeout stays below the request deadline`() throws {
        let thirtySecondTimeout = try #require(AgentBarCLI.serveProviderTimeout(requestTimeout: 30))
        let tenSecondTimeout = try #require(AgentBarCLI.serveProviderTimeout(requestTimeout: 10))
        ...
        #expect(AgentBarCLI.serveProviderTimeout(requestTimeout: 0) == nil)
        #expect(AgentBarCLI.serveProviderTimeout(requestTimeout: .infinity) == nil)
        // Finite deadlines stay strictly below the request timeout at every
        // value, including sub-second ones.
        let oneSecondTimeout = try #require(AgentBarCLI.serveProviderTimeout(requestTimeout: 1))
        let halfSecondTimeout = try #require(AgentBarCLI.serveProviderTimeout(requestTimeout: 0.5))
        ...
        let oversizedTimeout = try #require(AgentBarCLI.serveProviderTimeout(
    ...
    func `serve usage collection bounds a hung provider without blocking others`() async {
        ...
        let start = Date()
        let output = await AgentBarCLI.serveCollectUsageOutputs(
            providers: providers,
            providerTimeout: 0.1)
        { provider in
            if provider == .claude {
    ...
    func `serve cache uses stable Codex account identities`() {
        ...
        let firstKey = AgentBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: firstProjection)
        let reshapedKey = AgentBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: reshapedProjection)
        let replacementKey = AgentBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: replacement)
        let workspacePeerKey = AgentBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: workspacePeer)
        let storedBeforeKey = AgentBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: storedBeforeRefresh)
        let storedAfterKey = AgentBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
        ...
        #expect(AgentBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
    ...
    func `serve cache coalesces concurrent cache misses`() async {
        ...
        let responses = await withTaskGroup(of: CLILocalHTTPResponse.self) { group -> [CLILocalHTTPResponse] in
            for _ in 0..<5 {
                group.addTask {
                    await AgentBarCLI.cachedServeResponse(
                        key: "usage:",
                        cache: cache,
                        refreshInterval: 60,
                        requestTimeout: 1)
    ...
    func `serve cache prunes expired config token entries`() async {
        ...
        _ = await AgentBarCLI.cachedServeResponse(
    ...
    func `serve cache does not cache timeouts and recovers on next success`() async {
        ...
        let timeout = await AgentBarCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 60,
            requestTimeout: 0.01)
        {
            ...
            do {
                ...
                return AgentBarCLI.serveTimeoutResponse()
            }
        }
        ...
        let success = await AgentBarCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 60,
            requestTimeout: 1)
        ...
        let cached = await AgentBarCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 60,
            requestTimeout: 1)
    ...
    func `serve cache serves last good payload when refresh fails`() async {
        ...
        let first = await AgentBarCLI.cachedServeResponse(
        ...
        let failed = await AgentBarCLI.cachedServeResponse(
            key: "usage:antigravity",
            cache: cache,
            refreshInterval: 0.05,
            requestTimeout: 1)
        ...
        let recovered = await AgentBarCLI.cachedServeResponse(
            key: "usage:antigravity",
            cache: cache,
            refreshInterval: 0.05,
            requestTimeout: 1)
    ...
    func `cost refresh keeps fresh providers while replacing timed out rows`() async throws {
        ...
        let partial = await AgentBarCLI.cachedServeResponse(
            key: "cost:",
            cache: cache,
            refreshInterval: 0.01,
            requestTimeout: 1)
        ...
        wallClock.advance(by: 0.01)
        let timedOut = await AgentBarCLI.cachedServeResponse(
            key: "cost:",
            cache: cache,
            refreshInterval: 0.01,
            requestTimeout: 0.01)
    ...
    func `serve cache replaces only failed provider account rows`() async throws {
        ...
        let refreshed = await AgentBarCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 0.05,
            requestTimeout: 1)
    ...
    func `serve stale ttl is bounded and disabled without caching`() {
        #expect(AgentBarCLI.serveStaleTTL(refreshInterval: 0) == 0)
        #expect(AgentBarCLI.serveStaleTTL(refreshInterval: 1) == 300)
        #expect(AgentBarCLI.serveStaleTTL(refreshInterval: 60) == 600)
        #expect(AgentBarCLI.serveStaleTTL(refreshInterval: 1800) == 3600)
        #expect(AgentBarCLI.serveStaleTTL(refreshInterval: 86401) == 3600)
        #expect(AgentBarCLI.serveStaleTTL(refreshInterval: .infinity) == 3600)
    }
    ...
    func `serve helper idle window outlives the refresh cadence`() {
        #expect(AgentBarCLI.serveCLISessionIdleWindow(refreshInterval: 0) == 180)
        #expect(AgentBarCLI.serveCLISessionIdleWindow(refreshInterval: 60) == 180)
        #expect(AgentBarCLI.serveCLISessionIdleWindow(refreshInterval: 300) == 360)
    }
    ...
    func `serve request timeout zero disables the deadline`() async {
        ...
        let response = await AgentBarCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 0,
            requestTimeout: 0)

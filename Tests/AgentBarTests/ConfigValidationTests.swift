...
struct ConfigValidationTests {
    ...
    func `reports unsafe hook rule fields`() {
        ...
        let codes = Set(AgentBarConfigValidator.validate(config).map(\.code))
    ...
    func `reports hook workload limits`() {
        ...
        let config = AgentBarConfig(providers: [], hooks: HooksConfig(enabled: true, events: rules))
        let codes = Set(AgentBarConfigValidator.validate(config).map(\.code))
    ...
    func `sub2api accepts HTTPS and loopback HTTP base URLs`() {
        for host in ["https://sub2api.example.com", "http://127.0.0.1:8080"] {
            ...
            config.setProviderConfig(ProviderConfig(
                ...
                apiKey: "fixture",
                enterpriseHost: host))
            let invalidHostIssue = AgentBarConfigValidator.validate(config).first { issue in
    ...
    func `unsupported enterprise host warning lists every supported provider`() throws {
        ...
        let issue = try #require(AgentBarConfigValidator.validate(config).first(where: {
    ...
    func `config store default url honors environment override`() {
        let url = AgentBarConfigStore.defaultURL(environment: [
            AgentBarConfigStore.pathEnvironmentKey: "~/tmp/agentbar-test-config.json",
        ])

        #expect(url.path.hasSuffix("/tmp/agentbar-test-config.json"))
    }
    ...
    func `config store default url honors xdg config home`() throws {
        ...
        let url = AgentBarConfigStore.defaultURL(
            home: home,
            environment: [
                AgentBarConfigStore.xdgConfigHomeEnvironmentKey: xdgHome.path,
            ],
    ...
    func `config store default url ignores relative xdg config home`() throws {
        ...
        let url = AgentBarConfigStore.defaultURL(
            home: home,
            environment: [
                AgentBarConfigStore.xdgConfigHomeEnvironmentKey: "relative-config",
            ],
            fileManager: fileManager)
    ...
    func `config store default url creates in xdg default for new installs`() throws {
        ...
        let url = AgentBarConfigStore.defaultURL(home: home, environment: [:], fileManager: fileManager)
    ...
    private static func makeTemporaryHome() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("AgentBarConfigStoreTests-\(UUID().uuidString)", isDirectory: true)

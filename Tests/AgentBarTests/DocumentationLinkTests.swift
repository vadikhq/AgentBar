...
struct DocumentationLinkTests {
    ...
    func `local documentation paths normalize safely`() throws {
        let root = URL(filePath: "/tmp/AgentBar-documentation-links", directoryHint: .isDirectory)

        let target = try Self.localDocURL(
            for: "./docs/guide%20name.md?mode=print#topic",
            repositoryRoot: root)
        #expect(target.path == "/tmp/AgentBar-documentation-links/docs/guide name.md")

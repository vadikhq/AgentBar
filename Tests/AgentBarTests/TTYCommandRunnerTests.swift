...
struct TTYCommandRunnerEnvTests {
    ...
    func `codex status probe uses non persistent thread storage`() {
        let stateHome = URL(fileURLWithPath: "/tmp/agentbar status \"state\"", isDirectory: true)
        ...
        #expect(args.contains("experimental_thread_store={type=\"in_memory\",id=\"agentbar-status\"}"))
        #expect(args.contains("sqlite_home=\"/tmp/agentbar status \\\"state\\\"\""))
    }
    ...
    func `sets working directory when provided`() throws {
        ...
        let dir = fm.temporaryDirectory.appendingPathComponent("agentbar-tty-\(UUID().uuidString)", isDirectory: true)

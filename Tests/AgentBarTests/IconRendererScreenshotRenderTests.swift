...
final class IconRendererScreenshotRenderTests: XCTestCase {
    ...
    func test_renderSyntheticMergedWarpTransition() throws {
        guard let dir = ProcessInfo.processInfo.environment["AGENTBAR_WARP_ICON_PROOF_DIR"] else {
            throw XCTSkip("Set AGENTBAR_WARP_ICON_PROOF_DIR to render the merged Warp proof.")
        }
    ...
    func test_renderSyntheticSingleQuotaIcon() throws {
        guard let dir = ProcessInfo.processInfo.environment["AGENTBAR_ICON_SCREENSHOT_DIR"] else {
            throw XCTSkip("Set AGENTBAR_ICON_SCREENSHOT_DIR to render the synthetic icon proof.")
        }
        let directory = URL(fileURLWithPath: dir, isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)

...
struct CLISnapshotTests {
    ...
    func `defaults to usage when no command provided`() {
        #expect(AgentBarCLI.effectiveArgv([]) == ["usage"])
        #expect(AgentBarCLI.effectiveArgv(["--format", "json"]).first == "usage")
        #expect(AgentBarCLI.effectiveArgv(["usage", "--format", "json"]).first == "usage")
    }

...
struct ProviderPluginDetailsParityTests {
        ...
        engine: ProviderPluginEngineKind) async throws
    {
        ...
        let sourceURL = try #require(AgentBarCoreResources.bundle?.url(forResource: "openrouter", withExtension: "js"))
    ...
    func `OpenRouter cut-over honors default and overridden requests`(overridden: Bool) async throws {
        ...
        #expect(recorded[0].value(forHTTPHeaderField: "X-Title") == (overridden ? "AgentBar QA" : "AgentBar"))
        #expect(recorded[0].value(forHTTPHeaderField: "HTTP-Referer") ==
            (overridden ? "https://agentbar.example" : nil))

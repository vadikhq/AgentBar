...
struct CodexUsageFetcherFallbackTests {
    ...
    func `hung fixture recognizes both JSON slash encodings`(method: String) async throws {
        ...
        process.environment = ["AGENTBAR_TEST_RPC_REQUEST_PATH": requestPath]
        ...
        requestTimeoutSeconds: TimeInterval = 3.0) -> UsageFetcher
    {
        UsageFetcher(
            environment: [
                ...
                "AGENTBAR_TEST_RPC_REQUEST_PATH": stubCLIPath + ".requests",
            ],
        ...
        while IFS= read -r line; do
          case "$line" in
            ...
            *'"account/rateLimits/read"'*|*'"account\\/rateLimits\\/read"'*)
              printf '%s\\n' 'account/rateLimits/read' >> "$AGENTBAR_TEST_RPC_REQUEST_PATH"
              exec /bin/sleep 30
              ;;
            *)
              printf '%s\\n' '{"id":1,"result":{}}'

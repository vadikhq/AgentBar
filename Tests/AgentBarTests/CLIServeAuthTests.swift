...
struct CLIServeAuthTests {
    ...
    func `serve help documents dashboard token transport honestly`() {
        ...
        #expect(serve.contains("AGENTBAR_DASHBOARD_TOKEN"))
    ...
    func `serve host option parses and normalizes`() {
        #expect(AgentBarCLI.decodeServeHost(from: ParsedValues(
            positional: [],
            options: [:],
            flags: [])) == "127.0.0.1")
        #expect(AgentBarCLI.decodeServeHost(from: ParsedValues(
    ...
    func `serve flags parse through the real commander signature`() throws {
        ...
        #expect(AgentBarCLI.decodeServeHost(from: values) == "0.0.0.0")
        #expect(AgentBarCLI.decodeServeAllowPlainHTTP(from: values))
        #expect(AgentBarCLI.resolveDashboardToken(from: values, environment: [:]) == .token("secret"))
        #expect(AgentBarCLI.decodeServeHost(from: defaults) == "127.0.0.1")
        #expect(!AgentBarCLI.decodeServeAllowPlainHTTP(from: defaults))
        #expect(AgentBarCLI.resolveDashboardToken(from: defaults, environment: [:]) == .absent)
    }
    ...
    func `dashboard token resolution prefers the environment and rejects blanks`() {
        ...
        let envOverride = Dictionary(uniqueKeysWithValues: [
            (AgentBarCLI.dashboardTokenEnvironmentVariable, "ENV_VALUE"),
        ])

        #expect(AgentBarCLI.resolveDashboardToken(
            from: emptyValues,
            environment: [:]) == .absent)
        #expect(AgentBarCLI.resolveDashboardToken(
            from: flagValues,
        ...
        #expect(AgentBarCLI.resolveDashboardToken(
            from: emptyValues,
            environment: ["AGENTBAR_DASHBOARD_TOKEN": "  "])
            == .empty(source: "AGENTBAR_DASHBOARD_TOKEN"))
        #expect(AgentBarCLI.resolveDashboardToken(
    ...
    func `serve startup validation enforces the token and plain-http matrix`() {
        ...
        #expect(AgentBarCLI.validateServeStartup(
            host: "127.0.0.1",
    ...
    func `adding no-store preserves an existing cache-control header`() {
        ...
        let annotated = AgentBarCLI.addingNoStore(plain)
        let untouched = AgentBarCLI.addingNoStore(declared)

        #expect(annotated.extraHeaders.contains { $0 == ("Cache-Control", "no-store") })
        #expect(untouched.extraHeaders.count == 1)
        #expect(AgentBarCLI.addingNoStore(annotated).extraHeaders.count == 1)
    }

    @Test
    func `unauthorized response advertises bearer challenge and no-store`() {
        let response = AgentBarCLI.serveUnauthorizedResponse()

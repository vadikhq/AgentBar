...
struct AntigravityCLIHTTPSFetchStrategyTests {
    ...
    func `cli HTTPS resets session only for one-shot CLI runtime`() {
        ...
        // Long-lived CLI host (agentbar serve) keeps the warm session even at .cli runtime.
        ...
        env: [String: String] = [:]) -> ProviderFetchContext
    {
        ...
        effectiveEnv["HOME"] = effectiveEnv["HOME"] ??
            ...
            .appendingPathComponent("agentbar-antigravity-empty-home-\(UUID().uuidString)", isDirectory: true)
            .path

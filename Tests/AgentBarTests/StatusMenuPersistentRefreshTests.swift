...
struct StatusMenuPersistentRefreshTests {
    ...
    func `refresh monitor publishes compatible core and pins it until final reconciliation`() throws {
        ...
        if ProcessInfo.processInfo.environment["AGENTBAR_REFRESH_PROBE"] == "1" {
            print(
                "AGENTBAR_REFRESH_PROBE compatible-layout-shift refreshing=false pinned=" +
    ...
    func `refresh monitor keeps incompatible core frozen until reconciliation`() throws {
        ...
        if ProcessInfo.processInfo.environment["AGENTBAR_REFRESH_PROBE"] == "1" {
            print(
                "AGENTBAR_REFRESH_PROBE incompatible coreRows=\(refreshed.metrics.count) " +

...
struct CLIProviderSelectionTests {
    ...
    func `help includes gemini and all`() {
        let usage = AgentBarCLI.usageHelp(version: "0.0.0")
        ...
        #expect(usage.contains("agentbar usage --provider gemini"))
        #expect(usage.contains("agentbar usage --format json --provider all --pretty"))
        #expect(root.contains("agentbar --provider gemini"))
    }
    ...
    func `help advertises the toon output format`() {
        ...
        #expect(usage.contains("agentbar usage --format toon"))
        #expect(root.contains("agentbar --format toon"))
    }

    @Test
    func `provider selection respects override`() {
        let selection = AgentBarCLI.providerSelection(rawOverride: "gemini", enabled: [.codex, .claude])
        #expect(selection.asList == [.gemini])
    }

    @Test
    func `provider selection uses enabled providers when three or more are enabled`() {
        let selection = AgentBarCLI.providerSelection(
            rawOverride: nil,
    ...
    func `provider selection does not expand three enabled providers to all providers`() {
        ...
        let selection = AgentBarCLI.providerSelection(rawOverride: nil, enabled: enabled)
    ...
    func `provider selection accepts kiro alias`() {
        let selection = AgentBarCLI.providerSelection(rawOverride: "kiro-cli", enabled: [.codex])

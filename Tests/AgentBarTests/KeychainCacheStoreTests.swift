...
struct KeychainCacheStoreTests {
    ...
    func `tests suppress real keychain access by default`() {
        guard ProcessInfo.processInfo.environment["AGENTBAR_ALLOW_TEST_KEYCHAIN_ACCESS"] != "1" else { return }
    ...
    func `cache ACL trusts bundled app and CLI helper`() {
        let root = URL(fileURLWithPath: "/Applications/AgentBar.app")
        let executable = root.appendingPathComponent("Contents/MacOS/AgentBar")
        let helper = root.appendingPathComponent("Contents/Helpers/AgentBarCLI")
        let existing = Set([
            root.path,
            executable.path,
            helper.path,
    ...
    func `cache ACL refuses bare dev binaries without an app bundle`() {
        ...
        let executable = URL(fileURLWithPath: "/Users/dev/project/.build/debug/AgentBarCLI")

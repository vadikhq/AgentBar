...
struct CLIServeTimeoutTests {
    ...
    func `dashboard awaits pricing refresh while serve refreshes in background`() {
        #expect(!AgentBarCLI.dashboardCostRefreshesPricingInBackground)
        #expect(AgentBarCLI.serveCostRefreshesPricingInBackground)
    }

    @Test
    func `serve deadlines clamp once from request entry`() throws {
        #expect(AgentBarCLI.clampedServeRequestTimeout(.greatestFiniteMagnitude) == 86400)
        #expect(AgentBarCLI.clampedServeRequestTimeout(1e308) == 86400)
        #expect(AgentBarCLI.clampedServeRequestTimeout(-5) == 0)

        let startedAt = ContinuousClock().now
        let deadline = try #require(AgentBarCLI.serveRequestDeadline(
            startedAt: startedAt,
            requestTimeout: .greatestFiniteMagnitude))
        #expect(startedAt.duration(to: deadline) == .seconds(86400))
        #expect(AgentBarCLI.serveRequestDeadline(startedAt: startedAt, requestTimeout: 0) == nil)

        let requestDeadline = startedAt.advanced(by: .seconds(40))
        #expect(AgentBarCLI.serveCostProviderDeadline(
            startedAt: startedAt,
            providerTimeout: 30,
    ...
    func `late response warms the cache for the next same config request`() async {
        ...
        let next = await AgentBarCLI.cachedServeResponse(
            key: "dashboard:v1:snapshot:",
            cache: cache,
            refreshInterval: 60,
            requestTimeout: 30)
    ...
    func `provider timeout preserves healthy rows and cannot stack provider work`() async {
        ...
        let first = Task {
            await AgentBarCLI.serveCollectUsageOutputs(
    ...
    func `cost route variants cannot stack the same provider scan`() async {
        let clock = ServeManualDeadlineClock()
        let late = AgentBarCLI.makeCostPayload(provider: .claude, snapshot: nil, error: nil)
        ...
        let first = Task {
            await AgentBarCLI.serveCollectCostPayloads(
            ...
            { provider in
                ...
                return AgentBarCLI.makeCostPayload(provider: provider, snapshot: nil, error: nil)
            }
        }
        await blocked.waitForStarts(1)
        await clock.waitForPendingSleeps(1)
        ...
        cache: CLIServeResponseCache) async
    {
        ...
        _ = await cache.completeFetch(
            response,
            for: AgentBarCLI.serveCacheKey(operationKey: key, configToken: fingerprint),
...
extension CLIServeTimeoutTests {
    ...
    func `expired response returns stale immediately while background refresh commits`() async {
        ...
        let returned = await AgentBarCLI.cachedServeResponse(

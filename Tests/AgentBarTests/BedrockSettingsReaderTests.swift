...
struct BedrockSettingsReaderTests {
    ...
    func `explicit profile auth mode wins`() {
        let env = ["AGENTBAR_BEDROCK_AUTH_MODE": "profile"]
    ...
    func `hasCredentials in profile mode requires a profile name`() {
        let withProfile = ["AGENTBAR_BEDROCK_AUTH_MODE": "profile", "AWS_PROFILE": "work"]
        let withoutProfile = ["AGENTBAR_BEDROCK_AUTH_MODE": "profile"]

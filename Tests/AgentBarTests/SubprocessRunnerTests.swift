...
struct SubprocessRunnerTests {
    ...
    func `returns partial output when detached child keeps pipes open`() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("agentbar-subprocess-drain-\(UUID().uuidString)", isDirectory: true)
        ...
        with open(os.environ["AGENTBAR_TEST_CHILD_PID_FILE"], "w") as handle:
    ...
    func `timeout kills descendants that escape the process group`() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("agentbar-subprocess-tree-\(UUID().uuidString)", isDirectory: true)
        let childPIDFile = root.appendingPathComponent("child.pid")
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }

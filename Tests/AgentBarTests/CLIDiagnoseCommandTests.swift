...
struct CLIDiagnoseCommandTests {
    ...
    func `diagnose help describes generic JSON export`() {
        let help = AgentBarCLI.diagnoseHelp(version: "0.0.0")

        #expect(help.contains("agentbar diagnose --provider <name|all> --format json"))
        #expect(help.contains("agentbar diagnose --provider all --format json"))
    ...
    func `diagnose output writer creates parent directories`() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("AgentBarDiagnoseTests-\(UUID().uuidString)")
        defer { try? FileManager.default.removeItem(at: root) }

        let output = root.appendingPathComponent("nested/diagnostic.json")
        try AgentBarCLI.writeDiagnosticExport(#"{"provider":"minimax"}"#, to: output.path)
    ...
    func `generic diagnose auth summary requires complete Bedrock credentials`() {
        let partial = AgentBarCLI._diagnosticAuthSummaryForTesting(
            provider: .bedrock,
            account: nil,
        ...
        let complete = AgentBarCLI._diagnosticAuthSummaryForTesting(
            provider: .bedrock,
            account: nil,
            config: nil,
            environment: [

...
AgentBar reads AWS Cost Explorer for Bedrock spend and can compare the current month against an optional budget. When
permitted, it also reads CloudWatch for rolling 14-day Claude token and request totals in the configured region.

## Authentication

AgentBar supports two authentication modes, selected in Preferences → Providers → AWS Bedrock → Authentication.

### Access keys (default)

Provide static AWS credentials through Settings or the environment inherited by AgentBar/the CLI:
...
export AGENTBAR_BEDROCK_BUDGET="250"
```

### AWS profile

Resolve credentials from a named profile in `~/.aws/config` / `~/.aws/credentials` instead of pasting keys. Set the
profile name in Settings (or via `AWS_PROFILE`). AgentBar shells out to the AWS CLI
...
- AWS CLI v2 on your `PATH` (AgentBar also checks `/opt/homebrew/bin/aws`, `/usr/local/bin/aws`, and `~/.local/bin/aws`).
...
```bash
export AGENTBAR_BEDROCK_AUTH_MODE="profile"   # set automatically by Settings; "keys" or "profile"
...
- Budget: `AGENTBAR_BEDROCK_BUDGET`, when set to a positive dollar amount.
- Test override: `AGENTBAR_BEDROCK_API_URL` replaces the Cost Explorer endpoint; use HTTPS or loopback HTTP.
- Test override: `AGENTBAR_BEDROCK_CLOUDWATCH_API_URL` replaces the CloudWatch endpoint; use HTTPS or loopback HTTP.
...
```bash
agentbar --provider bedrock --source api
agentbar --provider bedrock --format json --pretty
```
...
- Confirm the credentials are visible to AgentBar.
...
Profile mode requires AWS CLI v2. Install it (e.g. `brew install awscli`) or point AgentBar at the binary with
`AWS_CLI_PATH`.

### Wrong region

Set `AWS_REGION` or `AWS_DEFAULT_REGION`. Bedrock usage is regional, but Cost Explorer itself is account-level; AgentBar still needs a signing region for the request.

## Key files

- `Sources/AgentBarCore/Providers/Bedrock/BedrockProviderDescriptor.swift`
- `Sources/AgentBarCore/Providers/Bedrock/BedrockSettingsReader.swift`
- `Sources/AgentBarCore/Providers/Bedrock/BedrockProfileCredentialProvider.swift`
- `Sources/AgentBarCore/Providers/Bedrock/BedrockUsageStats.swift`
- `Sources/AgentBarCore/Providers/Bedrock/BedrockAWSSigner.swift`

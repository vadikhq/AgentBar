...
AgentBar reads the policy attached to a ClawRouter API key. The menu card shows its monthly budget, spend, requests,
tokens, and provider breakdown. Provider rows come directly from ClawRouter, so the integration works with any routed
model provider configured there; AgentBar does not need a separate provider plugin for each route.

## Setup

Create a ClawRouter key with access to the routes you want, then store it in AgentBar:

```bash
printf '%s' "$CLAWROUTER_API_KEY" | agentbar config set-api-key --provider clawrouter --stdin
```

You can also paste the key in AgentBar Settings → Providers → ClawRouter. The hosted service is used by default:
...
root or `/v1`; AgentBar normalizes both to `/v1/usage`. Overrides must be HTTPS URLs or bare hosts normalized to HTTPS.
...
same AgentBar card aggregates them and lists the provider identifiers returned by `/v1/usage`.
...
AgentBar sends the key only to the validated ClawRouter endpoint. `/v1/usage` returns accounting metadata; AgentBar

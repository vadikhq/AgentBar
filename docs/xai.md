...
AgentBar reads the prepaid credit balance and daily USD spend of an xAI developer-platform team from xAI's documented
Management API.
...
it together with your **team ID** in AgentBar Settings → Providers → xAI. Inference API keys are not accepted by the
...
Both requests use `Authorization: Bearer <management key>`. AgentBar does not read browser cookies, console sessions,

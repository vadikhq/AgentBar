...
read_when:
  ...
  - Troubleshooting LiteLLM API-key usage in AgentBar
---
...
Configure it in Settings -> Providers -> LiteLLM, or in `~/.agentbar/config.json`:
...
`LITELLM_BASE_URL` may include `/v1`; AgentBar strips that suffix before calling LiteLLM management endpoints.
...
All requests use `Authorization: Bearer <apiKey>`. AgentBar does not request or store a LiteLLM master key.
...
endpoint. AgentBar validates returned user and team IDs against `/key/info` before displaying usage.

## Security

Treat LiteLLM keys as secrets. AgentBar stores configured keys only in provider config or token-account storage and

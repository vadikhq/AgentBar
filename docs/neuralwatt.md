...
```bash
printf '%s' "$NEURALWATT_API_KEY" | agentbar config set-api-key --provider neuralwatt --stdin
```

This trims the piped key, writes it to AgentBar's config file (`~/.config/agentbar/config.json`
by default, or the legacy `~/.agentbar/config.json` when already present), and enables Neuralwatt by
...
5. Paste the key into AgentBar's Neuralwatt provider settings

### Environment Variables

AgentBar also accepts these environment variables:
...
Set the key with `agentbar config set-api-key --provider neuralwatt --stdin`, add it in
...
quota endpoint is rate-limited to 1 request per second per customer; AgentBar refreshes on its

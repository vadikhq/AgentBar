...
3. Paste the header into the cookie field in AgentBar
...
Manus supports multiple accounts via the standard token-account mechanism. Add entries to `~/.agentbar/config.json` (`tokenAccounts`) with the full `Cookie:` header (containing `session_id=...`), then switch between accounts from the menu.

## CLI

```bash
agentbar usage --provider manus --verbose
```
...
Log in to [manus.im](https://manus.im) in a supported browser (Safari, Chrome, Firefox), then refresh AgentBar. Alternatively, set `MANUS_SESSION_TOKEN` or `MANUS_COOKIE`, or paste a cookie header in manual mode.

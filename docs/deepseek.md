...
AgentBar can use either a DeepSeek API key or a signed-in DeepSeek Platform session for the remaining credit balance.
Detailed cost and token usage comes from the Platform session; an API key cannot authenticate the private dashboard
endpoints.

## Data sources

1. **Optional API key** supplied via `DEEPSEEK_API_KEY` / `DEEPSEEK_KEY`, or selected from DeepSeek token accounts in `~/.agentbar/config.json`.
...
AgentBar resolves the Platform `userToken` in this order:
...
AgentBar checks every Chrome profile containing a parseable `userToken` against DeepSeek. Rejected or expired
sessions are omitted. Settings shows a **Chrome profile** picker containing only valid sessions. With no API key, one
valid session is selected automatically; with multiple valid sessions, AgentBar reuses the saved choice or asks once.
When an API key supplies the balance, a new or changed API credential requires an explicit session selection before
website usage is combined with it. AgentBar persists a stable browser/profile identifier, not an absolute home-directory path, and
...
If the selected session expires, AgentBar asks before switching to another valid profile.
...
- The API separates granted balance from topped-up balance; AgentBar labels these as granted vs. paid credit.
...
- The amount and cost requests run concurrently. After balance arrives, AgentBar waits up to five seconds for
...
- If total balance is zero, AgentBar shows an add-credits message. If balance is nonzero but `is_available` is false, it shows "Balance unavailable for API calls".
- There is no session or weekly window — DeepSeek does not expose per-window quota via API.
- Token-account selection injects the selected key into the fetch environment; otherwise AgentBar reads `DEEPSEEK_API_KEY` / `DEEPSEEK_KEY`.

## Key files

- `Sources/AgentBarCore/Providers/DeepSeek/DeepSeekProviderDescriptor.swift` (descriptor + fetch strategy)
- `Sources/AgentBarCore/Providers/DeepSeek/DeepSeekUsageFetcher.swift` (HTTP client + JSON parser)
- `Sources/AgentBarCore/Providers/DeepSeek/DeepSeekPlatformTokenImporter.swift` (Chrome Platform session import)
- `Sources/AgentBarCore/Providers/DeepSeek/DeepSeekSettingsReader.swift` (env var resolution)
- `Sources/AgentBar/Providers/DeepSeek/DeepSeekProviderImplementation.swift` (provider activation and token-account visibility)
- `Sources/AgentBarCore/TokenAccountSupportCatalog+Data.swift` (DeepSeek token-account injection)

...
read_when:
  - "Building a dashboard or adapter against AgentBar"
  - "Using agentbar dashboard from scripts"
...
AgentBar exposes one versioned, display-oriented snapshot contract through two transports:
...
`GET /` serves a self-contained web dashboard that polls `/dashboard/v1/snapshot`. The static HTML is always unauthenticated, including on non-loopback binds, because it contains no account data. When the snapshot route returns `401`, the page asks for the dashboard token, stores it in the browser under the localStorage key `agentbar.dashboardToken`, and sends it in the `Authorization` header on each snapshot request. The token is never added to the URL.

The browser keeps the last successfully merged snapshot in localStorage under `agentbar.lastSnapshot` and paints that data immediately on the next load, preserving the identity detail served by the configured mode. It then fetches the config-only shell and streams concurrent per-provider snapshot updates into the page as they finish. Signing out clears both the token and cached snapshot.

The UI does not change the transport threat model: `agentbar serve` is plain HTTP. Off-loopback, a token typed into the page transits the network in cleartext like every other request unless a TLS-terminating reverse proxy protects the connection.

## One-shot command semantics

- `agentbar dashboard` reads enabled providers from AgentBar config, emits them in stable order, and carries configured
...
- `agentbar serve` differs on one point: without `--identity` it follows the app's "Hide personal information" setting
  per request, so the web UI matches the menu UI. `agentbar dashboard` always defaults to full identity.
...
agentbar serve --dashboard-token YOUR_TOKEN
```

- `AGENTBAR_DASHBOARD_TOKEN` wins over `--dashboard-token` when both are set.
...
Transport is **plain HTTP**. There is no TLS in `agentbar serve`, which means:
...
1. **Loopback only (default).** `agentbar serve` binds `127.0.0.1`; nothing leaves the machine. Rejects non-loopback `Host` headers, so browser-based DNS-rebinding attacks cannot reach it either.
...
3. **Trusted network segment, cleartext accepted.** Bind a LAN address directly:
   ...
   ```bash
   AGENTBAR_DASHBOARD_TOKEN=... agentbar serve --host 0.0.0.0 --allow-plain-http --identity redacted
   ```
...
- `host.codexBarVersion`: AgentBar version when available.
...
- `providers[].enabled`: Whether the provider is enabled in AgentBar config.

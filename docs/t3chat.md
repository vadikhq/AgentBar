...
> **Unofficial integration:** T3 Chat does not publish an API for this data. AgentBar reads a cookie-authenticated
...
AgentBar imports your browser session cookie automatically. AgentBar sends the cookie only to
`https://t3.chat`.
...
5. Paste the full `curl` command into the **T3 Chat cookie** field in AgentBar settings.
...
```bash
# Show T3 Chat usage
agentbar usage --provider t3chat

# Or use the alias
agentbar usage --provider t3-chat
agentbar usage --provider t3
```

T3 Chat provides no token-cost data. The `usage --format json` output contains usage and identity
data, while `agentbar cost --provider t3chat` is unsupported.
...
| `T3 Chat session cookie is invalid or expired` | Stale or revoked session cookie | Sign out of T3 Chat, sign back in, then refresh AgentBar or repaste the cookie |
...
| `Could not parse T3 Chat usage` | T3 Chat changed its tRPC response shape | Open a AgentBar issue with a redacted response sample |
...
- `Sources/AgentBarCore/Providers/T3Chat/T3ChatProviderDescriptor.swift` — provider metadata and fetch pipeline
- `Sources/AgentBarCore/Providers/T3Chat/T3ChatUsageFetcher.swift` — tRPC request, cookie import, and cURL parsing
- `Sources/AgentBarCore/Providers/T3Chat/T3ChatUsageSnapshot.swift` — response decoding and window mapping
- `Sources/AgentBar/Providers/T3Chat/T3ChatProviderImplementation.swift` — settings pickers and bindings
- `Sources/AgentBar/Providers/T3Chat/T3ChatSettingsStore.swift` — cookie source and header persistence

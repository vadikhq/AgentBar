...
This solves the durable OAuth refresh problem without making AgentBar a second credential vault. It also avoids a
...
AgentBar has three account concepts today:

- The ambient Claude OAuth credential is routed from AgentBar's cache, Claude Code's credentials file, or Claude
  Code's Keychain item. It represents one active credential. Claude Code-owned expired credentials delegate refresh
  back to the CLI; AgentBar-owned cached credentials can refresh directly.
...
| --- | --- | --- | --- | --- |
| First-party OAuth account vault | AgentBar | High | New login, refresh, storage, revocation, migration, and security surface | Defer |
...
| Existing token accounts | AgentBar config | Low for OAuth | Access token expires without refresh metadata | Keep for current cookie/API-key uses |
...
envelope. AgentBar does not need
...
- AgentBar never reads `claude-swap` storage, Claude Code storage, environment credentials, or Keychain entries. The
...
last refresh, adapter errors, and a link to the upstream project; AgentBar should not install or update it.

...
3. Refresh provider usage. AgentBar resolves `arkcli` through `ARKCLI_PATH`, the login-shell/host `PATH`, and standard install locations.
...
- If the key is valid but rate-limit headers are missing, AgentBar shows the key as active and links to the dashboard for details.

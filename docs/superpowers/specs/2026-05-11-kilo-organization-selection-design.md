...
The Kilo provider in AgentBar always queries the personal account. Users who belong to one or more Kilo organizations cannot see organization-level credits, KiloPass usage, or plan info. Kilo's own clients (VS Code extension, CLI) let users pick "Personal" or any organization they belong to and route requests with an `X-KILOCODE-ORGANIZATIONID` header.

Goal: let AgentBar users opt in to seeing one or more Kilo organizations alongside their personal account.

## Non-goals

- Menu-bar org switcher. Selection lives in Preferences only.
- Editing org membership from AgentBar (read-only consumer of Kilo's org list).
- Per-org auth (AgentBar reuses the same API key/CLI session; Kilo's gateway scopes via header).
...
- AgentBar's Kilo provider currently calls `https://app.kilo.ai/api/trpc` with procedures `user.getCreditBlocks`, `kiloPass.getState`, `user.getAutoTopUpPaymentMethod`. The `X-KILOCODE-ORGANIZATIONID` header is transport-level and must work for the same procedures.
- Existing AgentBar patterns to reuse:
  ...
  - `~/.agentbar/config.json` per-provider entry for persisted state.
...
- `KiloOrganization` (Sendable, Codable, Equatable) in `Sources/AgentBarCore/Providers/Kilo/`:
  - `id: String`
  - `name: String`
...
Persistence:
- Add `organizations: [KiloOrganization]?` and `enabledOrganizationIds: [String]?` to the Kilo provider's entry in `~/.agentbar/config.json`.
...
- Unit tests (in `Tests/AgentBarTests`):
...
- CLI snapshot test (`Tests/AgentBarTests/CLIRendererTests` or similar): `agentbar-cli kilo` shows one section per active scope when orgs are enabled.
...
- CLI command for org switching (`agentbar-cli` consumes the same settings).

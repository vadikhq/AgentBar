---
summary: "AgentBar release checklist: package, sign, notarize, appcast, and asset validation."
read_when:
  - Starting a AgentBar release
  - Updating signing/notarization or appcast steps
  - Validating release assets or Sparkle feed
---

# Release process (AgentBar)

SwiftPM-only; package/sign/notarize manually (no Xcode project). The Sparkle feed is served from `appcast.xml` on `main`, with enclosures hosted on GitHub Releases. Checklist below merges Trimmy’s release flow with AgentBar specifics.

**Must read first:** open the master macOS release guide at `~/Projects/agent-scripts/docs/RELEASING-MAC.md` alongside this file and reconcile any differences in favor of AgentBar specifics before starting a release.

## Expectations
- When someone says “release AgentBar”, do the entire end-to-end flow: bump versions/CHANGELOG, build, sign and notarize, upload the zip to the GitHub release, generate/update the appcast with the new signature, publish the tag/release, and verify the enclosure URL responds with 200/OK and installs via Sparkle (no 404s or stale feeds).

### Release automation notes (Scripts/release.sh)
- Rebuilds both release architectures and notarizes before publishing; set `AGENTBAR_FORCE_CLEAN=1` when a cache-free SwiftPM rebuild is required.
...
- Sparkle keys: public key expectation is in `.mac-release.env`; AgentBar still uses the older shared AGCY key, so the manifest includes the local Dropbox fallback path. `SPARKLE_PRIVATE_KEY_FILE` overrides it.
...
```
./Scripts/build_icon.sh Icon.icon AgentBar
```
...
- Avoid `unzip` — it can add AppleDouble `._*` files that break the sealed signature and trigger “app is damaged”. Use Finder or `ditto -x -k AgentBar-<ver>.zip /Applications`. If Gatekeeper complains, delete the app bundle, re-extract with `ditto`, then `spctl -a -t exec` to verify.
- Manual sanity check before uploading: `find AgentBar.app -name '._*'` should return nothing; then `spctl --assess --type execute --verbose AgentBar.app` and `codesign --verify --deep --strict --verbose AgentBar.app` should both pass on the packaged bundle.

## iCloud sync (CloudKit)
Release builds embed `Scripts/profiles/AgentBar-DeveloperID.provisionprofile` at `Contents/embedded.provisionprofile` and claim the iCloud entitlements (`Scripts/package_app.sh` does both automatically; it fails hard if the profile file is missing). The profile expires 2044-07-29; Gatekeeper re-validates it at every launch.

Schema changes: any new record type or field in `Sources/AgentBar*/Sync/` must be reflected in `Scripts/cloudkit/schema.ckdb` and deployed **before** shipping the build:
```
...
```
./Scripts/make_appcast.sh AgentBar-macos-universal-0.1.0.zip \
...
## Homebrew (Cask)
AgentBar ships a Homebrew **Cask** in `../homebrew-tap`. When installed via Homebrew, AgentBar disables Sparkle and the app
...
## Checklist (quick)
- [ ] Read both this file and `~/Projects/agent-scripts/docs/RELEASING-MAC.md`; resolve any conflicts toward AgentBar’s specifics.
...
- [ ] Generate Sparkle appcast via `Scripts/release.sh` or `Scripts/make_appcast.sh`; use `SPARKLE_PRIVATE_KEY_FILE` only if overriding Keychain signing.
  ...
  - Generate the appcast + HTML release notes: `./Scripts/make_appcast.sh AgentBar-macos-universal-<ver>.zip https://raw.githubusercontent.com/steipete/CodexBar/main/appcast.xml`
...
- [ ] Homebrew tap: wait for the Release CLI workflow to update `../homebrew-tap/Casks/codexbar.rb` (app zip url + sha256) and `../homebrew-tap/Formula/codexbar.rb` (CLI tarball urls + sha256), then verify:
  ...
  - `brew install --cask steipete/tap/codexbar && open -a AgentBar`
...
- [ ] Release pages: title format `AgentBar <version>`, notes as Markdown list (no stray blank lines)
- [ ] Changelog/release notes are user-facing: avoid internal-only bullets (build numbers, script bumps) and keep entries concise
- [ ] Download uploaded `AgentBar-macos-universal-<ver>.zip`, unzip via `ditto`, run, and verify signature (`spctl -a -t exec -vv AgentBar.app` + `stapler validate`)
...
- [ ] Keep a previous signed build in `/Applications/AgentBar.app` to test Sparkle delta/full update to the new release
- [ ] Manual Gatekeeper sanity: after packaging, `find AgentBar.app -name '._*'` is empty, `spctl --assess --type execute --verbose AgentBar.app` and `codesign --verify --deep --strict --verbose AgentBar.app` succeed
- [ ] For Sparkle verification: if replacing `/Applications/AgentBar.app`, quit first, replace, relaunch, and test update

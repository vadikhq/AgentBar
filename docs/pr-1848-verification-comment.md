...
- isolated `.claude/.credentials.json` and AgentBar config
- synthetic `claude` executable that records benign discovery separately from `/status` touch

The packaged `AgentBarCLI` exited 3 with the MCP-only guidance, no `/status` or browser/open canary appeared, and the user keychain search list was unchanged. The packaged `AgentBar.app` then exercised the synthetic CLI with `--version`, stayed running for five seconds after discovery, and still sent no `/status` or browser/open touch.

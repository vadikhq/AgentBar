...
1. Resolve a Factory API key from, in order:
   - `~/.agentbar/config.json` `providers[].apiKey` for `factory` (also via Settings or
     `agentbar config set-api-key --provider factory`)
   - `FACTORY_API_KEY`
...
1) **Cached cookie header** (Keychain cache `com.vadikhq.agentbar.cache`, account `cookie.factory`).
2) **Stored session** (`~/Library/Application Support/AgentBar/factory-session.json`).
...
## CLI
```bash
printf '%s' "$FACTORY_API_KEY" | agentbar config set-api-key --provider factory --stdin
agentbar usage --provider factory --source api
agentbar usage --provider factory --source web
```
...
## Session storage
- File: `~/Library/Application Support/AgentBar/factory-session.json`
...
## Troubleshooting
- Missing API key: set `FACTORY_API_KEY`, Settings → Droid → API key, or `agentbar config set-api-key --provider factory`.
...
## Key files
- `Sources/AgentBarCore/Providers/Factory/FactoryProviderDescriptor.swift`
- `Sources/AgentBarCore/Providers/Factory/FactorySettingsReader.swift`
- `Sources/AgentBarCore/Providers/Factory/FactoryStatusProbe.swift`
- `Sources/AgentBarCore/Providers/Factory/FactoryLocalStorageImporter.swift`

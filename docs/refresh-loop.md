...
## Adaptive mode
- `AdaptiveRefreshPolicy` (`Sources/AgentBar/AdaptiveRefreshPolicy.swift`) is a pure function of an `Input`
...
- An allowed scan runs `ps -axo ... command=` to inspect the running-process list and identify Codex/Claude, then runs
  ...
  the Agent Sessions UI is off, AgentBar discards the resulting session records and retains only the latest `Date`.

...
## Scripts
- `Scripts/package_app.sh`: builds host arch with ad-hoc signing by default; set `ARCHES="arm64 x86_64"` for universal. Verifies slices. Stable-certificate packaging requires explicit `AGENTBAR_SIGNING=identity` plus `APP_IDENTITY`.
...
## Bundle contents
- `AgentBarWidget.appex` is built by `WidgetExtension/AgentBarWidgetExtension.xcodeproj` as a real macOS app extension, then bundled with app-group entitlements.
- `AgentBarCLI` copied to `AgentBar.app/Contents/Helpers/` for symlinking.

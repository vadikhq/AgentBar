...
AgentBar's Azure OpenAI provider validates that a configured deployment is reachable. It does not read Azure spend,
...
Settings -> Providers -> Azure OpenAI stores those values in the shared AgentBar config. The same values can also be
...
```bash
printf '%s' "$AZURE_OPENAI_API_KEY" | agentbar config set-api-key --provider azure-openai --stdin
```
...
AgentBar sends a minimal chat-completions request to validate the deployment:
...
Set `AZURE_OPENAI_API_VERSION` to override the API version. When it is set to `v1`, AgentBar uses Azure's
...
HTTPS. AgentBar rejects explicit `http://` endpoints, user info, and encoded host-delimiter tricks before attaching the
`api-key` header.

Endpoint paths are preserved. AgentBar avoids duplicating a trailing `/openai` for dated API versions or a trailing
...
```bash
agentbar usage --provider azure-openai
agentbar usage --provider azureopenai
agentbar usage --provider aoai
```

## Key files

- `Sources/AgentBarCore/Providers/AzureOpenAI/AzureOpenAIProviderDescriptor.swift`
- `Sources/AgentBarCore/Providers/AzureOpenAI/AzureOpenAISettingsReader.swift`
- `Sources/AgentBarCore/Providers/AzureOpenAI/AzureOpenAIUsageFetcher.swift`
- `Sources/AgentBar/Providers/AzureOpenAI/AzureOpenAIProviderImplementation.swift`
- `Tests/AgentBarTests/AzureOpenAIUsageFetcherTests.swift`

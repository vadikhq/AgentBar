...
This document is the current-state parity reference for Claude behavior in AgentBar.
...
This behavior is owned by `Sources/AgentBarCore/Providers/Claude/ClaudeProviderDescriptor.swift`
...
- CLI-side Claude token-account behavior:
  - OAuth token account changes the effective source mode from `auto` to `oauth`, disables cookie mode, omits a
    manual cookie header, and injects `AGENTBAR_CLAUDE_OAUTH_TOKEN`.
...
This behavior is implemented in `Sources/AgentBarCore/Providers/Claude/ClaudeUsageFetcher.swift`
...
- `Tests/AgentBarTests/ClaudeBaselineCharacterizationTests.swift`
- `Tests/AgentBarTests/ClaudeOAuthFetchStrategyAvailabilityTests.swift`
- `Tests/AgentBarTests/ClaudeUsageTests.swift`
- `Tests/AgentBarTests/TokenAccountEnvironmentPrecedenceTests.swift`
- `Tests/AgentBarTests/SettingsStoreCoverageTests.swift`

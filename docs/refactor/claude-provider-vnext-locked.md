...
- CLI token-account behavior must remain consistent in both app and `AgentBarCLI`.
...
- `.agentbar` expired credentials: direct refresh endpoint path.
...
- Owner semantics parity remains intact across all sub-phases (`claudeCLI`, `agentbar`, `environment`).
...
| PR-06 | OAuth decomposition | Extract repository, refresher, and delegated-controller seams from `ClaudeOAuthCredentialsStore` while preserving owner semantics. | R3, R4, R7, R9 | Cache/fingerprint/prompt/owner suites green (`claudeCLI`, `agentbar`, `environment`). |
...
| R4 | High | Owner semantics drift | Preserve exact owner-to-refresh mapping: `.claudeCLI` delegated, `.agentbar` direct refresh, `.environment` no refresh. |

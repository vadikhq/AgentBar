...
1. Config token (`~/.config/agentbar/config.json` or legacy `~/.agentbar/config.json` → `providers[].apiKey`).
...
### Config location
- New installs: `~/.config/agentbar/config.json`
- Legacy installs: `~/.agentbar/config.json`
- Override for scripts/tests: `AGENTBAR_CONFIG=/path/to/config.json`
...
- Check:
  ...
  agentbar usage --provider zai --account Team
  ```
...
For BigModel China team usage, AgentBar needs the `Bigmodel-Organization` and `Bigmodel-Project` request headers:
...
- Endpoint overrides must be explicit HTTPS URLs or bare hosts/paths that AgentBar normalizes to HTTPS. Explicit
...
- Live API checks return success with empty limits when one of the selectors is missing, so AgentBar treats both
...
- AgentBar's Usage Dashboard action follows the configured API region.
...
## Key files
- `Sources/AgentBarCore/Resources/Plugins/zai.js` (quota parsing and window mapping)
- `Sources/AgentBarCore/Providers/Zai/ZaiProviderDescriptor.swift`
- `Sources/AgentBarCore/Providers/Zai/ZaiSettingsReader.swift`
- `Sources/AgentBar/ZaiTokenStore.swift` (legacy migration helper)
- `Tests/AgentBarTests/ProviderPluginDetailsParityTests.swift` (quota fixtures, including credit limits)

...
> **Unsupported integration:** AgentBar uses Notion's internal, cookie-authenticated `/api/v3` endpoints.
...
workspaces make the endpoint answer `{"status":"not_applicable"}`, and AgentBar surfaces that as a clear
...
AgentBar imports your browser session cookie automatically and sends it only to `https://app.notion.com`.
...
5. Paste the full `curl` command into the **Notion cookie** field in AgentBar settings.
...
AgentBar sends two POST requests per refresh, both to `https://app.notion.com`:
...
| AgentBar window | Notion field | Notes |
| --- | --- | --- |
...
Both bars carry an expected-usage estimate on the card and in `agentbar usage`, reading `n% in deficit`
...
Notion publishes a status page at <https://status.notion.so/>; AgentBar links to it but does not poll
components.

...
DeepInfra is API-only. AgentBar uses DeepInfra's documented billing endpoints and does not send model prompts or account data to any other service.

## Setup

1. Create an API key in the [DeepInfra dashboard](https://deepinfra.com/dash).
2. In AgentBar, open **Settings > Providers > DeepInfra > API tokens** and add the key.

You can instead set `DEEPINFRA_API_KEY` or `DEEPINFRA_TOKEN` in AgentBar's environment. `DEEPINFRA_API_KEY` takes precedence when both are present.

## Data source

AgentBar sends the key as a bearer token to:
...
DeepInfra represents prepaid funds as a negative `stripe_balance`; AgentBar converts that to a positive “available” amount. A positive value is shown as money owed.
...
- If the account has a positive spending limit, AgentBar shows billing-cycle spend against that limit.
...
AgentBar never logs the API key or raw billing response.

...
the cheap/local or the dearer/cloud tier serves it. AgentBar polls the gateway's read-only
...
The integration is read-only. AgentBar never sends prompts through the gateway, never calls
...
in to: enable the provider in AgentBar Settings → Providers → Wayfinder. The default gateway
...
- Routing split over the last 30 days: requests per configured route (up to 5, by request
  ...
  asserting which route is "local," so AgentBar never guesses.
...
If the gateway is not running, AgentBar reports that it could not be reached and suggests

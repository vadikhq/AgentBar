---
summary: "AgentBar CLI configuration commands for provider toggles, API keys, and isolated config files."
read_when:
  - Using agentbar config from scripts or CI
...
`agentbar config` edits the same resolved config file used by the app's Settings → Providers pane.
New installs use `~/.config/agentbar/config.json`; absolute `XDG_CONFIG_HOME` paths and `AGENTBAR_CONFIG` are
supported, and existing `~/.agentbar/config.json` installs keep using the legacy file when no XDG config exists.
...
agentbar config providers --json --pretty
```
...
These are persistent app/CLI settings. They are different from `agentbar usage --provider grok`, which is a one-shot
command override and does not edit config.

If every provider is disabled, `agentbar usage` with no `--provider` prints no text output, and
`agentbar usage --json` prints `[]`. Passing `--provider <name>` still fetches that provider for the one command.
...
```bash
printf '%s' "$OPENROUTER_API_KEY" | agentbar config set-api-key --provider openrouter --stdin --no-enable
```
...
printf '%s' "$ANTHROPIC_ADMIN_KEY" | agentbar config set-api-key --provider claude --stdin
printf '%s' "$DEEPGRAM_API_KEY" | agentbar config set-api-key --provider deepgram --stdin
...
in the AgentBar config file.

## Isolated config files

For tests, demos, and CI, point AgentBar at a temporary config file:

```bash
export AGENTBAR_CONFIG=/tmp/agentbar-config.json
agentbar config enable --provider grok
agentbar config providers --json --pretty
```
...
```bash
agentbar cost --provider codex --days 90
agentbar cost --provider claude --days 180 --format json --pretty
```

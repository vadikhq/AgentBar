...
AgentBar reads subscription quota windows and pay-as-you-go balance from ZenMux's documented Management API.
...
AgentBar Settings → Providers → ZenMux. Standard ZenMux inference API keys are not accepted by these endpoints.
...
```bash
printf '%s' "$ZENMUX_MANAGEMENT_API_KEY" | agentbar config set-api-key --provider zenmux --stdin
```
...
Both requests use `Authorization: Bearer <ZENMUX_MANAGEMENT_API_KEY>`. AgentBar does not read ZenMux browser cookies,

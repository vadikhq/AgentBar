...
AgentBar shows Groq organization usage and spend from the **console.groq.com** dashboard API, read through your
...
- Because the JWT cookie expires quickly and is refreshed by the SPA only while a console tab is open, AgentBar
...
- Each activity row is per-model, per-day: `cost`, `n_context_tokens_total`, `n_non_cached_context_tokens_total`
  (cached = context − non-cached), `n_generated_tokens_total`, `num_requests`. AgentBar aggregates these into daily
...
```bash
GROQ_SESSION_TOKEN=<stytch_session> agentbar usage --provider groq --json
```

## Prometheus metrics (Enterprise, optional)

- Requires an Enterprise API key stored in `~/.agentbar/config.json` (Settings → Providers → Groq) or `GROQ_API_KEY`.
...
- Console fetch: `Sources/AgentBarCore/Providers/Groq/GroqConsoleFetcher.swift`
- Session import + JWT resolution: `Sources/AgentBarCore/Providers/Groq/GroqConsoleSession.swift`
- Stytch refresh: `Sources/AgentBarCore/Providers/Groq/GroqConsoleStytch.swift`
- Snapshot / cost-history projection: `Sources/AgentBarCore/Providers/Groq/GroqConsoleUsageSnapshot.swift`
- Prometheus fallback: `Sources/AgentBarCore/Providers/Groq/GroqUsageFetcher.swift`
- Provider wiring: `Sources/AgentBarCore/Providers/Groq/GroqProviderDescriptor.swift`

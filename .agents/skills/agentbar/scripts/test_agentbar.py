...
SCRIPT = Path(__file__).with_name("agentbar")
MODULE = runpy.run_path(str(SCRIPT), run_name="agentbar_skill")
...
def make_env(root: Path, *, install: bool = True) -> tuple[dict[str, str], Path]:
    binary = root / "AgentBar.app" / "Contents" / "Helpers" / "AgentBarCLI"
    binary.parent.mkdir(parents=True)
    binary.write_text(FAKE, encoding="utf-8")
    binary.chmod(0o755)
    env = os.environ.copy()
    env["AGENTBAR_SKIP_DISCOVERY"] = "1"
    env["AGENTBAR_TIMEOUT"] = "3"
    env["FAKE_LOG"] = str(root / "argv.log")
    env["FAKE_VERSION_STDOUT"] = "AgentBar 1.2.3\n"
    ...
    if install:
        env["AGENTBAR_BIN"] = str(binary)
    else:
        env.pop("AGENTBAR_BIN", None)
...
class AgentBarSkillTests(unittest.TestCase):
    def test_help_is_small_and_read_only(self) -> None:
        result = helper("--help", env=os.environ.copy())
        self.assertEqual(result.returncode, 0)
        self.assertIn("AgentBar read. JSON out. No writes.", result.stdout)
    ...
    def test_doctor_reports_version_and_raw_validation_shape(self) -> None:
        ...
        self.assertEqual(payload["version"], "AgentBar 1.2.3")

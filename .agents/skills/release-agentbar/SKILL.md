---
name: release-agentbar
description: "AgentBar release: versioning, notarization, appcast, Homebrew, post-release bump."
---
...
- repo: `~/Projects/agentbar`
- release script: `Scripts/release.sh`
- signing/notarization: `Scripts/sign-and-notarize.sh`
- appcast: `Scripts/make_appcast.sh`, `appcast.xml`
- release assets: `AgentBar-macos-universal-<version>.zip`, `AgentBar-macos-universal-<version>.dSYM.zip`
- packaged app: `AgentBar.app`
...
```bash
tmux new-session -d -s agentbar-release 'op run --account my.1password.com --env-file /tmp/agentbar-release-op.env -- Scripts/release.sh'
tmux attach -t agentbar-release
```
...
If widget metadata generation times out, `AGENTBAR_WIDGET_METADATA_TIMEOUT_SECONDS=600` is a known-good floor.

AgentBar CLI tarballs are not produced by `Scripts/release.sh` itself. The GitHub release event triggers `.github/workflows/release-cli.yml`, which builds and uploads:

- `AgentBarCLI-v<version>-macos-arm64.tar.gz`
- `AgentBarCLI-v<version>-macos-x86_64.tar.gz`
- `AgentBarCLI-v<version>-linux-aarch64.tar.gz`
...
spctl --assess --type execute --verbose AgentBar.app
```
...
/usr/libexec/PlistBuddy -c 'Print :CFBundleVersion' AgentBar.app/Contents/Info.plist
```

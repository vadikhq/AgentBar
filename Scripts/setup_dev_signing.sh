...
# Check if we already have a AgentBar development certificate
CERT_NAME="AgentBar Development"
...
openssl req -x509 -newkey rsa:4096 -sha256 -days 3650 \
    -nodes -keyout /tmp/agentbar-dev.key -out /tmp/agentbar-dev.crt \
    -config "$TEMP_CONFIG" 2>/dev/null

# Convert to PKCS12 format
openssl pkcs12 -export -out /tmp/agentbar-dev.p12 \
    -inkey /tmp/agentbar-dev.key -in /tmp/agentbar-dev.crt \
    -passout pass: 2>/dev/null

# Import into keychain
security import /tmp/agentbar-dev.p12 -k ~/Library/Keychains/login.keychain-db -T /usr/bin/codesign -T /usr/bin/security

# Clean up temporary files
rm -f /tmp/agentbar-dev.{key,crt,p12}

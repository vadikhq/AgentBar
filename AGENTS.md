...
- `Sources/AgentBar`: Swift 6 menu bar app (usage/credits probes, icon renderer, settings). Keep changes small and reuse existing helpers.
- `Tests/AgentBarTests`: XCTest coverage for usage parsing, status probes, icon patterns; mirror new logic with focused tests.
...
- Package locally: `./Scripts/package_app.sh` to refresh `AgentBar.app`, then restart with `pkill -x AgentBar || pkill -f AgentBar.app || true; cd /Users/steipete/Projects/agentbar && open -n /Users/steipete/Projects/agentbar/AgentBar.app`.
...
## Testing Guidelines
- Add/extend XCTest cases under `Tests/AgentBarTests/*Tests.swift` (`FeatureNameTests` with `test_caseDescription` methods).
...
- Prefer CLI/focused tests over app-bundle live tests when behavior can be verified without relaunching AgentBar.
- Never run tests/checks or ad-hoc validation that can display macOS Keychain prompts. Live provider probes, browser-cookie imports, `agentbar usage` against real accounts, and real SecItem reads must be explicitly requested; otherwise use parser tests, stubs, test stores, or `KeychainNoUIQuery`.
...
- Menu bar automation: capture the target screen first and verify the AgentBar icon is visibly onscreen. Reject `click-extra` success when coordinates fall outside display bounds; hidden menu extras are not click proof.
...
- To guarantee the right bundle is running after a rebuild, use: `pkill -x AgentBar || pkill -f AgentBar.app || true; cd /Users/steipete/Projects/agentbar && open -n /Users/steipete/Projects/agentbar/AgentBar.app`.

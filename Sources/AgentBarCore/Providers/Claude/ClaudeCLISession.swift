...
actor ClaudeCLISession {
    ...
    private static let log = AgentBarLog.logger(LogCategories.provider(.claude, scope: "cli"))
    private static let probeSessionIDFilename = ".agentbar-session-id"
    ...
    enum SessionError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .outputTooLarge: "Claude CLI session produced more output than AgentBar can safely process."
            }
        }
    }
        ...
        environment: [String: String]) throws
    {
        ...
        let disableWatchdog = sessionIdentity.environment["AGENTBAR_DISABLE_CLAUDE_WATCHDOG"] == "1"
        if !disableWatchdog,
           resolvedURL.lastPathComponent == "claude",
           let watchdog = TTYCommandRunner.locateBundledHelper("AgentBarClaudeWatchdog")
        {

...
public enum ClaudeOAuthCredentialsStore {
    ...
    public static let environmentTokenKey = "AGENTBAR_CLAUDE_OAUTH_TOKEN"
    public static let environmentScopesKey = "AGENTBAR_CLAUDE_OAUTH_SCOPES"
    ...
    public static let environmentClientIDKey = "AGENTBAR_CLAUDE_OAUTH_CLIENT_ID"
    ...
    static let log = AgentBarLog.logger(LogCategories.provider(.claude, scope: "usage"))
    ...
    private static let pendingAgentBarOAuthKeychainCacheClearKey =
        "ClaudeOAuthPendingAgentBarOAuthKeychainCacheClearV1"
    ...
    private static var sharedDefaults: UserDefaults {
        UserDefaults(suiteName: "com.vadikhq.agentbar") ?? .standard
    }

    private static let pendingAgentBarOAuthKeychainCacheClearStore: ClaudeOAuthPendingCacheClearStore =
        ClaudeOAuthPendingCacheClearUserDefaultsStore(
            ...
            domain: "com.vadikhq.agentbar",
            key: ClaudeOAuthCredentialsStore.pendingAgentBarOAuthKeychainCacheClearKey)
    ...
    private static let isolatedTestCredentialsURL = FileManager.default.temporaryDirectory
        .appendingPathComponent("agentbar-tests-\(UUID().uuidString)", isDirectory: true)
    ...
    private struct Repository {
            ...
            clearInvalidCache: Bool = true) throws -> ClaudeOAuthCredentialRecord
        {
            try self.context.run {
                ...
                if ClaudeOAuthCredentialsStore.shouldUseAgentBarOAuthKeychainCache,
                   !ClaudeOAuthCredentialsStore.hasPendingAgentBarOAuthKeychainCacheClear(
                ...
                switch ClaudeOAuthCredentialsStore.loadAgentBarOAuthKeychainCache(
                    profileIdentifier: profileIdentifier)
                {
                ...
                case .temporarilyUnavailable:
                    ...
                    lastError = ClaudeOAuthCredentialsError.readFailed("AgentBar cache is temporarily unavailable.")
                case .missing:
                    break
                }
        ...
        private func prepareCachePolicy(environment: [String: String]) -> String {
            ...
            if !ClaudeOAuthCredentialsStore.shouldUseAgentBarOAuthKeychainCache {
                ClaudeOAuthCredentialsStore.markPendingAgentBarOAuthKeychainCacheClear(
                    profileIdentifier: profileIdentifier)
            }
            ...
            profileIdentifier: String) -> ClaudeOAuthCredentialRecord?
        {
            ...
            guard case let .found(entry) = ClaudeOAuthCredentialsStore.loadAgentBarOAuthKeychainCache(
                profileIdentifier: profileIdentifier),
        ...
        {
            guard owner == .agentbar else { return owner }
            ...
            // keeps the chain CLI-owned. Only positive absence lets AgentBar keep its mirror chain alive.
            return .claudeCLI
        }
            ...
            environment: [String: String]) -> Bool
        {
            ...
            case .unavailable, .notApplicable:
                ...
                // and only positive proof of a signed-out CLI releases the chain to AgentBar.
            ...
            environment: [String: String] = ProcessInfo.processInfo.environment) -> Bool
        {
            self.context.run {
                ...
                if ClaudeOAuthCredentialsStore.shouldUseAgentBarOAuthKeychainCache {
                    if ClaudeOAuthCredentialsStore.hasPendingAgentBarOAuthKeychainCacheClear(
                        profileIdentifier: profileIdentifier)
                    {
        ...
        func shouldAttemptFreshnessSyncFromClaudeKeychain(cached: ClaudeOAuthCredentialRecord) -> Bool {
            ...
            case .onlyOnUserAction:
                if ProviderInteractionContext.current != .userInitiated {
                    if ProcessInfo.processInfo.environment["AGENTBAR_DEBUG_CLAUDE_OAUTH_FLOW"] == "1" {
                        ClaudeOAuthCredentialsStore.log.debug(
    ...
    /// - AgentBar-owned credentials refresh directly via token endpoint.
    ...
    /// Save refreshed credentials to AgentBar's keychain cache
        ...
        profileIdentifier: String)
    {
        ...
        self.saveToCacheKeychain(
            ...
            historyOwnerIdentifier: historyOwnerIdentifier,
            profileIdentifier: profileIdentifier)
        self.log.debug("Saved refreshed credentials to AgentBar keychain cache")
    }
    ...
    /// Check if AgentBar has cached credentials (in memory or keychain cache)
    ...
    private static func saveToCacheKeychain(
        ...
        profileIdentifier: String)
    {
        guard self.shouldUseAgentBarOAuthKeychainCache else {
            self.markPendingAgentBarOAuthKeychainCacheClear(profileIdentifier: profileIdentifier)
            return
        }
        let entry = CacheEntry(
            ...
            historyOwnerIdentifier: historyOwnerIdentifier,
            profileIdentifier: profileIdentifier)
        self.currentPendingAgentBarOAuthKeychainCacheClearStore.withCacheTransaction(
            profileIdentifier: profileIdentifier,
    ...
    private static func clearCacheKeychain(profileIdentifier: String) -> Bool {
        if self.shouldUseAgentBarOAuthKeychainCache {
    ...
    private static func loadAgentBarOAuthKeychainCache(
        profileIdentifier: String) -> KeychainCacheStore.LoadResult<CacheEntry>
    {
        guard self.shouldUseAgentBarOAuthKeychainCache else { return .missing }
    ...
    private static func clearProfileCacheKeychain(profileIdentifier: String) -> Bool {
        guard self.shouldUseAgentBarOAuthKeychainCache else { return false }
    ...
    private static var shouldUseAgentBarOAuthKeychainCache: Bool {
        ClaudeOAuthKeychainPromptPreference.storedMode() != .never
    }

    private static func markPendingAgentBarOAuthKeychainCacheClear(profileIdentifier: String) {
        self.currentPendingAgentBarOAuthKeychainCacheClearStore.withCacheTransaction(
            profileIdentifier: profileIdentifier,
    ...
    private static func hasPendingAgentBarOAuthKeychainCacheClear(profileIdentifier: String) -> Bool {
        self.currentPendingAgentBarOAuthKeychainCacheClearStore.isPending(profileIdentifier: profileIdentifier)
    }

    #if DEBUG
    static var hasPendingAgentBarOAuthKeychainCacheClearForTesting: Bool {
        self.currentPendingAgentBarOAuthKeychainCacheClearStore.isPending
    }
    #endif

    private static var currentPendingAgentBarOAuthKeychainCacheClearStore: ClaudeOAuthPendingCacheClearStore {
        #if DEBUG
        ...
        #endif
        return self.pendingAgentBarOAuthKeychainCacheClearStore
    }
    ...
    static var keychainAccessAllowed: Bool {
        ...
        // replaces its ACL, so any permission granted to AgentBar is inherently temporary and can cause recurring
        // macOS password dialogs. Production AgentBar therefore reads the foreign item only after the user
    ...
    private static func credentialsProfileIdentifier(for credentialsURL: URL) -> String {
        ...
        let material = Data("agentbar:claude-oauth-cache-profile:v1\0\(path)".utf8)
    ...
    static func _resetCredentialsFileTrackingForTesting() {
        ...
        if self.taskPendingCacheClearStoreOverride != nil {
            self.currentPendingAgentBarOAuthKeychainCacheClearStore.withCacheTransaction { pending in
                pending = false
            }
        }
    }
...
extension ClaudeOAuthCredentialsStore {
    ...
    /// AgentBar's caches. This is used to avoid triggering a second OS keychain dialog during the OAuth retry.
    @discardableResult
    ...
    /// Updates AgentBar's keychain cache with the new credentials.

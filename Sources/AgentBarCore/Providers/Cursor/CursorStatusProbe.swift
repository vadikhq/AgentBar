...
public enum CursorCookieImporter {
    ...
    /// profile that AgentBar cannot read. Ordinary background imports remain stricter.
...
public enum CursorStatusProbeError: LocalizedError, Sendable {
    ...
    static let safariFullDiskAccessHint =
        "If you use Safari, grant AgentBar Full Disk Access in System Settings ▸ Privacy & Security."

    public var errorDescription: String? {
        switch self {
        case .notLoggedIn:
            #if os(macOS)
            "Not logged in to Cursor. Please log in via the AgentBar menu."
            #else
            "Not logged in to Cursor. Paste a Cookie header copied from cursor.com into "
                + "~/.config/agentbar/config.json (legacy: ~/.agentbar/config.json)."
            #endif
        ...
        case .noSessionCookie:
            ...
            "No Cursor session found. \(Self.safariFullDiskAccessHint) "
                ...
                + "You can also sign in to Cursor from the AgentBar menu (Add / switch account)."
            #else

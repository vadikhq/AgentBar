...
public enum OllamaUsageError: LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        switch self {
        case .missingAPIKey:
            "Missing Ollama API key. Set apiKey in ~/.agentbar/config.json or OLLAMA_API_KEY."
        case .notLoggedIn:
        ...
        case .safariCookieAccessDenied:
            "Safari cookies need Full Disk Access for AgentBar (System Settings > Privacy & Security)."
        ...
        case let .browserCookieDecryptionDisabled(browserName):
            "\(browserName) cookie decryption is disabled in AgentBar; enable Keychain access and refresh."
        }
    }
}
...
public enum OllamaAPIUsageFetcher {
        ...
        now: Date = Date()) async throws -> OllamaAPIUsageSnapshot
    {
        ...
        request.setValue("AgentBar/1.0", forHTTPHeaderField: "User-Agent")

...
/// process. ``agy`` is an interactive REPL, not a query command, so AgentBar
...
struct AntigravityCLIHTTPSFetchStrategy: ProviderFetchStrategy {
    ...
    private static let log = AgentBarLog.logger(LogCategories.provider(.antigravity))
    ...
    struct WarmAgyDependencies {
        ...
        /// The pid of an ``agy`` that AgentBar itself spawned and manages through
        ...
        /// long-lived `agy`, or another AgentBar host) has no such accounting.
        ...
        dependencies: WarmAgyDependencies) async throws -> AntigravityStatusSnapshot?
    {
        ...
        // Also exclude any `agy` AgentBar itself spawned and manages: reusing it
    ...
    static func liveWarmAgyDependencies() -> WarmAgyDependencies {
        WarmAgyDependencies(
            ...
            ownedPID: {
                // The pid of the `agy` AgentBar manages through the shared
        ...
        async throws -> ProviderFetchResult
    {
        ...
        // process (an IDE, a long-lived `agy`, or another AgentBar host), so
        // AgentBar must not manage its lifecycle, idle timeout, or
        // `resetAfterFetch` teardown. Those apply only to processes AgentBar
    ...
    /// Spawn (or reuse AgentBar's own warm) `agy` session and wait for the CLI
    ...
    static func shouldResetSessionAfterFetch(_ context: ProviderFetchContext) -> Bool {
        // Long-lived hosts (the app, `agentbar serve`) keep the warm `agy`

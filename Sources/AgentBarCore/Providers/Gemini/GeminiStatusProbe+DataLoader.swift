...
extension GeminiStatusProbe {
        ...
        -> @Sendable (URLRequest) async throws -> (Data, URLResponse)
    {
        { request in
            do {
                ...
                AgentBarLog.logger(LogCategories.provider(.gemini, scope: "probe"))
    ...
    private static func curlDataLoader(for request: URLRequest) async throws -> (Data, URLResponse) {
        ...
        let tempDir = fileManager.temporaryDirectory
            .appendingPathComponent("agentbar-gemini-curl-\(UUID().uuidString)", isDirectory: true)
    ...
    private static let curlHTTPStatusMarker = "__AGENTBAR_HTTP_STATUS__:"

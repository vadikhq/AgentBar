...
let openAIDashboardScrapeScript = """
    ...
    (() => {
      ...
      const usageBreakdownJSON = (() => {
        try {
          if (window.__agentbarUsageBreakdownJSON) return window.__agentbarUsageBreakdownJSON;
          ...
          window.__agentbarUsageBreakdownJSON = json;
          window.__agentbarUsageBreakdownDebug = json ? null : JSON.stringify(debug);
          return json;
        } catch {
          return null;
        }
      ...
      const usageBreakdownDebug = (() => {
        try {
          return window.__agentbarUsageBreakdownDebug || null;
        } catch {
          return null;
        }
      })();
      ...
      try {
        ...
        if (header) {
          ...
          if (rows.length === 0 && !window.__agentbarDidScrollToCredits) {
        ...
        } else if (rows.length === 0 && !window.__agentbarDidScrollToCredits && scrollHeight > viewportHeight * 1.5) {
          rows = allTableRows();
          if (rows.length > 0) {
            creditsHeaderPresent = true;
        ...
        if (rows.length === 0 && !window.__agentbarDidScrollToCredits && scrollHeight > viewportHeight * 1.5) {

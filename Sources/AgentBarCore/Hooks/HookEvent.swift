...
/// The quota/provider state changes AgentBar can run external commands for.
///
...
public struct HookEvent: Codable, Sendable, Equatable {
    ...
    public func environmentVariables() -> [String: String] {
        var env: [String: String] = [
            "AGENTBAR_EVENT": self.event.rawValue,
            "AGENTBAR_PROVIDER": self.provider,
            "AGENTBAR_TIMESTAMP": Self.iso8601String(self.timestamp),
        ]
        if let account { env["AGENTBAR_ACCOUNT"] = account }
        if let window { env["AGENTBAR_WINDOW"] = window }
        if let usagePercent { env["AGENTBAR_USAGE_PERCENT"] = Self.number(usagePercent) }
        if let used { env["AGENTBAR_USED"] = Self.number(used) }
        if let limit { env["AGENTBAR_LIMIT"] = Self.number(limit) }
        if let resetAt { env["AGENTBAR_RESET_AT"] = Self.iso8601String(resetAt) }
        if let status { env["AGENTBAR_STATUS"] = status }
        return env
    }

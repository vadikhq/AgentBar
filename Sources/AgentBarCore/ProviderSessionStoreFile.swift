...
enum ProviderSessionStoreFile {
    static let isolationEnvironmentKey = "AGENTBAR_TEST_SESSION_FILE_ISOLATION"

    private static let testDirectory = FileManager.default.temporaryDirectory
        .appendingPathComponent("agentbar-session-tests-\(UUID().uuidString)", isDirectory: true)
        ...
        }) -> URL
    {
            ...
            || KeychainTestSafety.isRunningUnderTests(processName: processName, environment: environment)
        {
            ...
            applicationSupportDirectory().appendingPathComponent("AgentBar", isDirectory: true)
        }

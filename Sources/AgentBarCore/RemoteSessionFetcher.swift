...
public struct RemoteSessionFetcher: Sendable {
    public static let bundledCLIFallback = "/Applications/AgentBar.app/Contents/Helpers/AgentBarCLI"
        ...
        environment: [String: String] = ProcessInfo.processInfo.environment) async
    {
        ...
        let command = "agentbar sessions focus \(Self.shellQuote(sessionID)) || " +
    ...
    /// wrapper, but a GUI-launched AgentBar inherits a minimal `PATH` (`/usr/bin:/bin`)

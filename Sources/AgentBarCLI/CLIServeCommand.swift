...
struct ServeOptions: CommanderParsable {
    ...
    @Option(
        ...
        help: "Bearer token for /dashboard/v1/snapshot (prefer AGENTBAR_DASHBOARD_TOKEN)")
...
extension AgentBarCLI {
    ...
    static func runServe(_ values: ParsedValues) async {
        ...
        do {
            try await server.run {
                Self.writeStderr("AgentBar server listening on http://\(bindHost):\(port)\n")
    ...
    static let dashboardTokenEnvironmentVariable = "AGENTBAR_DASHBOARD_TOKEN"
    ...
    static func loadServeConfigSnapshot(
        configStore: AgentBarConfigStore = AgentBarConfigStore()) throws -> CLIServeConfigSnapshot
    {
        let config = try configStore.load() ?? AgentBarConfig.makeDefault()
    ...
    static func serveConfigCacheToken(for config: AgentBarConfig) throws -> String {
    ...
    static func cachedServeResponse(
        ...
        cache: CLIServeResponseCache,
        refreshInterval: TimeInterval,
        requestTimeout: TimeInterval = AgentBarCLI.defaultServeRequestTimeout,
        configFingerprint: String = "",
    ...
    private static func serveProviderSelection(
        rawProvider: String?,
        config: AgentBarConfig) throws -> ProviderSelection
    {

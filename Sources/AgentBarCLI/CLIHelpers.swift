...
extension AgentBarCLI {
    static func decodeProvider(from values: ParsedValues, config: AgentBarConfig) -> ProviderSelection {
    ...
    static func resetTimeDisplayStyleFromDefaults() -> ResetTimeDisplayStyle {
        let domains = [
            "com.vadikhq.agentbar",
            "com.vadikhq.agentbar.debug",
        ]
    ...
    static func loadConfig(output: CLIOutputPreferences) -> AgentBarConfig {
        let store = AgentBarConfigStore()
        do {
            if let existing = try store.load() {
                return existing
            }
            return AgentBarConfig.makeDefault()
        } catch {

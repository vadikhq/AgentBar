...
enum CLIServeStartupError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case let .missingDashboardToken(host):
            "--dashboard-token (or AGENTBAR_DASHBOARD_TOKEN) is required for non-loopback --host '\(host)'."
...
extension AgentBarCLI {
    /// Startup validation for `agentbar serve` binding and dashboard-token flags.

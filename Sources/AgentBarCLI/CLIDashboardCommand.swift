...
/// The CLI command encodes it directly while `agentbar serve` wraps it in the
...
struct DashboardSnapshotProducer: Sendable {
    ...
    let collectCost: @Sendable ([UsageProvider], AgentBarConfig) async -> [CostPayload]
    let now: @Sendable () -> Date
    var collectClaudeSwapAccounts: @Sendable (AgentBarConfig) async -> DashboardClaudeSwapCollection? = { _ in nil }
    ...
    {
        let selection = requestedProviders.map(ProviderSelection.custom) ?? AgentBarCLI.providerSelection(
            rawOverride: nil,
        ...
        let costPayloads = await self.collectCost(
            AgentBarCLI.costProviders(from: selection),
            config)
    ...
    static func live(context: DashboardSnapshotContext) -> Self {
        Self(
            ...
            collectCost: { providers, config in
                ...
                { provider, cursorCookieHeaderOverride in
                    do {
                        ...
                        return AgentBarCLI.makeCostPayload(provider: provider, snapshot: snapshot, error: nil)
                    } catch {
                        return AgentBarCLI.makeCostPayload(provider: provider, snapshot: nil, error: error)
                    }
                }
            },
            now: { Date() },
            collectClaudeSwapAccounts: { config in
                ...
                guard AgentBarCLI.dashboardClaudeSwapIsEligible(config: config) else { return nil }
            ...
            weeklyWorkDays: { AgentBarCLI.weeklyProgressWorkDaysFromDefaults() })
    }
}
...
extension AgentBarCLI {
    ...
    static func dashboardClaudeSwapIsEligible(config: AgentBarConfig) -> Bool {
    ...
    static func writeDashboardSnapshotAtomically(_ data: Data, toPath path: String) throws {
        ...
        let staged = directory.appendingPathComponent(
            ".\(url.lastPathComponent).agentbar-dashboard-\(UUID().uuidString)", isDirectory: false)

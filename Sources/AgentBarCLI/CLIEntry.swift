...
enum AgentBarCLI {
    static func main() async {
        if AgentBarCoreResourceSmoke.isRequested() {
            #if canImport(Darwin)
            Darwin.exit(AgentBarCoreResourceSmoke.run())
            #elseif canImport(Glibc)
            Glibc.exit(AgentBarCoreResourceSmoke.run())
            #elseif canImport(Musl)
            Musl.exit(AgentBarCoreResourceSmoke.run())
            #endif
        }
    ...
    private static func bootstrapLogging(path: [String], values: ParsedValues) {
        AgentBarLog.bootstrapIfNeeded(self.loggingConfiguration(path: path, values: values))
    }

    static func loggingConfiguration(path: [String], values: ParsedValues) -> AgentBarLog.Configuration {
        ...
        let destination: AgentBarLog.Destination = path == ["diagnose"] ? .discard : .stderr
        return .init(destination: destination, level: level, json: isJSON)
    }

    static func resolvedLogLevel(verbose: Bool, rawLevel: String?) -> AgentBarLog.Level {
        AgentBarLog.parseLevel(rawLevel) ?? (verbose ? .debug : .error)
    }

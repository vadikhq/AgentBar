...
struct AgentBarUsageWidgetView: View {
...
struct AgentBarHistoryWidgetView: View {
    @Environment(\.widgetFamily) private var family
    let entry: AgentBarWidgetEntry
...
struct AgentBarCompactWidgetView: View {
...
struct AgentBarSwitcherWidgetView: View {
    @Environment(\.widgetFamily) private var family
    let entry: AgentBarSwitcherEntry
...
enum WidgetColors {
    static func color(for instanceID: ProviderInstanceID) -> Color {
        ...
        // The widget cannot read ~/.agentbar/config.json, so it resolves the user override from the

...
final class StatusItemController: NSObject, NSMenuDelegate, StatusItemControlling {
    ...
    private nonisolated static let statusItemAccessibilityTitle = "AgentBar"
    private nonisolated static let debugStatusItemAccessibilityTitle = "AgentBar Debug"
    ...
    let loginLogger = AgentBarLog.logger(LogCategories.login)
    let menuLogger = AgentBarLog.logger(LogCategories.app)
    ...
    @objc private func handleProviderConfigDidChange(_ notification: Notification) {
        ...
        {
            if let config = notification.userInfo?["config"] as? AgentBarConfig {

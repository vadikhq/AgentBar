...
final class OpenAICreditsPurchaseWindowController: NSWindowController, WKNavigationDelegate, WKScriptMessageHandler {
    ...
    private static let logHandlerName = "agentbarLog"
    private static let debugLogURL = URL(fileURLWithPath: NSTemporaryDirectory())
        .appendingPathComponent("agentbar-buy-credits.log")
    private static let autoStartScript = """
    (() => {
      if (window.__agentbarAutoBuyCreditsStarted) return 'already';
      const log = (...args) => {
        try {
          window.webkit?.messageHandlers?.agentbarLog?.postMessage(args);
        } catch {}
      };
      ...
      const startNextPolling = (initialDelay = 500, interval = 500, maxAttempts = 90) => {
        if (window.__agentbarNextPolling) return;
        window.__agentbarNextPolling = true;
      ...
      const observeNextButton = () => {
        if (window.__agentbarNextObserver || !window.MutationObserver) return;
        const observer = new MutationObserver(() => {
          if (clickNextIfReady(1)) {
            observer.disconnect();
            window.__agentbarNextObserver = null;
          }
        });
        observer.observe(document.body, { subtree: true, childList: true, attributes: true });
        window.__agentbarNextObserver = observer;
      };
      ...
      if (findAndClick()) {
        window.__agentbarAutoBuyCreditsStarted = true;
        startNextPolling();
        observeNextButton();
        logDialogButtons();
        return 'clicked';
    ...
    private let logger = AgentBarLog.logger(LogCategories.creditsPurchase)

...
final class UsageStore {
    ...
    @ObservationIgnored let sessionQuotaLogger = AgentBarLog.logger(LogCategories.sessionQuota)
    // Provider-specific by design: OpenAI web and Augment runtime diagnostics have dedicated app-owned log streams.
    @ObservationIgnored let openAIWebLogger = AgentBarLog.logger(LogCategories.provider(.openai, scope: "web"))
    @ObservationIgnored let tokenCostLogger = AgentBarLog.logger(LogCategories.tokenCost)
    @ObservationIgnored let augmentLogger = AgentBarLog.logger(LogCategories.provider(.augment))
    @ObservationIgnored let providerLogger = AgentBarLog.logger(LogCategories.providers)
    @ObservationIgnored let adaptiveRefreshLogger = AgentBarLog.logger(LogCategories.adaptiveRefresh)
...
extension UsageStore {
    func debugDumpClaude() async {
        ...
        let url = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("agentbar-claude-probe.txt")
    ...
    func dumpLog(toFileFor provider: UsageProvider) async -> URL? {
        ...
        let filename = "agentbar-\(provider.rawValue)-probe.txt"
        ...
        fallbackCookieHeader: String) async -> ClaudeDebugLogConfiguration
    {
        await MainActor.run {
            ...
            return ClaudeDebugLogConfiguration(
                runtime: AgentBarCore.ProviderRuntime.app,

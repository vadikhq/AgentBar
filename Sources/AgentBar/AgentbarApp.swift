...
enum AgentBarLaunchMode: Equatable {
    case application
    case hookEvent

    static func resolve(arguments: [String]) -> Self {
        // Other AgentBar installations can leave this app path registered in ~/.codex/hooks.json.
...
@main
enum AgentBarEntryPoint {
    @MainActor
    static func main() {
        // Packaging launch smoke check (#2738): force the resource loads that
        // trapped in 0.48.0 and exit before any AppKit/UI setup.
        if AgentBarCoreResourceSmoke.isRequested() {
            exit(AgentBarCoreResourceSmoke.run())
        }
        guard AgentBarLaunchMode.resolve(arguments: CommandLine.arguments) == .application else {
            return
        }
        AgentBarApp.main()
    }
...
struct AgentBarApp: App {
    ...
    init() {
        ...
        let storedLevel = AgentBarLog.parseLevel(UserDefaults.standard.string(forKey: "debugLogLevel")) ?? .verbose
        let level = AgentBarLog.parseLevel(env["AGENTBAR_LOG_LEVEL"]) ?? storedLevel
        AgentBarLog.bootstrapIfNeeded(.init(
            destination: .oslog(subsystem: "com.vadikhq.agentbar"),
            level: level,
            json: false))
        ...
        AgentBarLog.logger(LogCategories.app).info(
            "AgentBar starting",
            metadata: [
                "version": version,
                "build": build,
        ...
        AgentBarLog.setLogLevel(settings.debugLogLevel)
    ...
    private static func applyLanguagePreference(from settings: SettingsStore) {
        ...
        resetAgentBarLocalizationCache()
    }
}
...
final class SparkleUpdaterController: NSObject, UpdaterProviding, SPUUpdaterDelegate {
    ...
    {
        _ = updater
        AgentBarLog.logger(LogCategories.app).debug(
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private let confettiLogger = AgentBarLog.logger(LogCategories.confetti)
    ...
    func applicationShouldOpenUntitledFile(_ sender: NSApplication) -> Bool {
        // AgentBar lives in the menu bar and has no untitled document to open at launch or on reopen.
        false
    }
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if !self.hasInstalledLimitResetObservers {
            NotificationCenter.default.addObserver(
                ...
                name: .agentbarSessionLimitReset,
                object: nil)
            NotificationCenter.default.addObserver(
                self,
                selector: #selector(self.handleWeeklyLimitResetNotification(_:)),
                name: .agentbarWeeklyLimitReset,
                object: nil)
    ...
    func openSettings(pane: SettingsPane?) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            guard let settingsWindowController = self.settingsWindowController else {
                ...
                AgentBarLog.logger(LogCategories.app).error("Settings window controller was not configured")
                return
            }
        ...
        fallbackVerificationRetriesRemaining: Int)
    {
        guard let mainMenu = NSApp.mainMenu else {
            if missingItemRetriesRemaining > 0 {
                ...
                AgentBarLog.logger(LogCategories.app).error("Application menu unavailable for Settings validation")
            }
            return
        }
        ...
        case let .repaired(previousCount, installedFallback):
            AgentBarLog.logger(LogCategories.app).warning(
        ...
        case .missingApplicationMenu:
            if missingItemRetriesRemaining > 0 {
                ...
                AgentBarLog.logger(LogCategories.app).error("Could not repair application Settings menu")
            }
        }
    }
    ...
    private func ensureStatusController() {
        ...
        AgentBarLog.logger(LogCategories.app)
    ...
    private func installDebugMemoryPressureObserverIfNeeded() {
        ...
        self.debugMemoryPressureObserver = DistributedNotificationCenter.default().addObserver(
            forName: .agentbarDebugSimulateMemoryPressure,
            object: nil,
            queue: .main)
    ...
    private func handleDebugMemoryPressureNotification(rawLevel: String?, shouldSeedCaches: Bool) {
        ...
        AgentBarLog.logger(LogCategories.memoryPressure).info(

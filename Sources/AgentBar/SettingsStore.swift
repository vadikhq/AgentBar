...
final class SettingsStore {
    ...
    @ObservationIgnored let configStore: AgentBarConfigStore
    @ObservationIgnored let antigravityOAuthCredentialsStore: AntigravityOAuthCredentialsStore
    @ObservationIgnored var config: AgentBarConfig
    ...
    init(
        ...
        configStore: AgentBarConfigStore = AgentBarConfigStore(),
        ...
        performInitialProviderDetection: Bool = !SettingsStore.isRunningTests)
    {
        ...
        let legacyStores = AgentBarConfigMigrator.LegacyStores(
        ...
        let config = AgentBarConfigMigrator.loadOrMigrate(
        ...
        AgentBarLog.setFileLoggingEnabled(self.debugFileLoggingEnabled)
...
extension SettingsStore {
        ...
        hadPreviousInstallationState: Bool) -> SettingsDefaultsState
    {
        ...
        let debugLogLevelRaw = userDefaults.string(forKey: "debugLogLevel") ?? AgentBarLog.Level.verbose.rawValue
...
extension SettingsStore {
    var configSnapshot: AgentBarConfig {
        _ = self.configRevision
        return self.config
    }

    func updateProviderState(config: AgentBarConfig) {
    ...
    func setProviderEnabled(provider: UsageProvider, metadata _: ProviderMetadata, enabled: Bool) {
        AgentBarLog.logger(LogCategories.settings).debug(

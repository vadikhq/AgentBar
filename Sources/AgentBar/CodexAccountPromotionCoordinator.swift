    ...
    static func mapUserFacingError(_ error: Error) -> CodexSystemAccountPromotionUserFacingError {
        ...
        if let error = error as? CodexAccountPromotionError {
            ...
            case .targetManagedAccountNotFound:
                L("That account is no longer available in AgentBar. Refresh the account list and try again.")
            case .targetManagedAccountAuthMissing:
                L("AgentBar could not find saved auth for that account. Re-authenticate it and try again.")
            case .targetManagedAccountAuthUnreadable:
                L("AgentBar could not read saved auth for that account. Re-authenticate it and try again.")
            case .liveAccountUnreadable:
                L("AgentBar could not read the current system account on this Mac.")
            case .liveAccountMissingIdentityForPreservation:
                L("AgentBar could not safely preserve the current system account before switching.")
            case .liveAccountAPIKeyOnlyUnsupported:
                L("AgentBar can't replace a system account that is signed in with an API key only setup.")
            case .displacedLiveManagedAccountConflict:
                L(
                    "AgentBar found another managed account that already uses the current system account. " +
                        "Resolve the duplicate account before switching.")
            case .displacedLiveImportFailed:
                L("AgentBar could not save the current system account before switching.")
            case .managedStoreCommitFailed:
                L("AgentBar could not update managed account storage.")
            case .liveAuthSwapFailed:
                L("AgentBar could not replace the live Codex auth on this Mac.")
            }

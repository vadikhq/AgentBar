...
extension SettingsStore {
    ...
    func reloadTokenAccounts() {
        let log = AgentBarLog.logger(LogCategories.tokenAccounts)
    ...
    func openTokenAccountsFile() {
        do {
            ...
            AgentBarLog.logger(LogCategories.tokenAccounts).error("Failed to persist config: \(error)")
            return
        }
        ...
        removedCredentials: AntigravityOAuthCredentials)
    {
        do {
            ...
            AgentBarLog.logger(LogCategories.tokenAccounts).warning(
